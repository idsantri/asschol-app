...
<script setup>
import { Provinsi } from '@/models/Address';
...
const onSubmit = async (e) => {
    ...
    try {
        ...
        let response = null;
        if (isNew) {
            response = await Provinsi.create(data);
            emit('successCreate', response?.provinsi);
        } else {
            response = await Provinsi.update(props.data.id, data);
...
const onDelete = async () => {
    ...
    try {
        loading.value = true;
        await Provinsi.remove(props.data.id);
